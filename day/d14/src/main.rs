    ...
    loop {
        ...
        while (0<= pos.0) && (pos.0 < rows) && (0<= pos.1) && (pos.1 < cols) {
            for dir in directions{
                ...
                if (0<= candidate.0) && (candidate.0 < rows) && (0<= candidate.1) && (candidate.1 < cols) && grid[candidate.0 as usize][candidate.1 as usize] {
                    pos = candidate;
                    break
                }
            }
...
fn main() {
    ...
    if let Ok(lines) = read_lines("input.txt") {
        ...
        let mut grid = vec![vec![false; (max_j + 1 - min_j) as usize]; (max_i + 1 - min_i) as usize];
        if let Ok(lines) = read_lines("input.txt") {
        
        for line in lines {
            if let Ok(ip) = line {
                let mut corners = ip.split(" -> ")
                .map(|x| x.split(',').map(|x| x.parse::<i32>().unwrap())).map(|mut x| ((x.next().unwrap()-min_i) as usize, (x.next().unwrap()-min_j) as usize)).into_iter();
